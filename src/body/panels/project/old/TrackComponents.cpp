    ...
    : editViewState(evs), track(move(t))
{
  ...
  if (auto at = dynamic_cast<te::AudioTrack*>(track.get()))
  {
    inputButton.onClick = [this, at] {
      ...
      if (ext::trackHasInput(*at))
      {
        bool ticked = ext::isInputMonitoringEnabled(*at);
        m.addItem(
            1000,
            "Input Monitoring",
            true,
      ...
      if (res == 1000)
      {
        ext::enableInputMonitoring(
            *at,
            !ext::isInputMonitoringEnabled(*at));
      }
      else if (res >= 100)
      {
        int id = 100;
      ...
      track(move(t))
{
  ...
  //    if (auto plugin = pickPlugin(track->edit))
